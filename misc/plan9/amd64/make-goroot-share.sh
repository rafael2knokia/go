#!/usr/bin/env bash
# Cross-build plan9/amd64 Go and prepare a trimmed GOROOT for QEMU.
#
# By default, also packages the staged tree into a real FAT32 image via
# mtools (mformat/mcopy), which is much more reliable than qemu's vvfat
# under amd64 + KVM (vvfat corrupts the cluster chain of large files under
# concurrent reads/writes).  Set NO_FAT_IMAGE=1 to skip that step.

set -euo pipefail

repo_root=$(git rev-parse --show-toplevel)
stage=${1:-"$repo_root/tmp-9front-amd64-goroot"}
...
echo "building std and cmd for plan9/amd64"
GOOS=plan9 GOARCH=amd64 "$go_cmd" install std cmd
...
cp -R "$repo_root/bin/plan9_amd64" "$stage/bin/"
cp "$repo_root/bin/plan9_amd64/go" "$stage/bin/go"
if [ -f "$repo_root/bin/plan9_amd64/gofmt" ]; then
	cp "$repo_root/bin/plan9_amd64/gofmt" "$stage/bin/gofmt"
fi

cp -R "$repo_root/pkg/tool/plan9_amd64" "$stage/pkg/tool/"
if [ -d "$repo_root/pkg/include" ]; then
	cp -R "$repo_root/pkg/include" "$stage/pkg/"
fi

if [ -f "$repo_root/misc/plan9/arm64/loopback-close-probe.go" ]; then
	echo "building loopback close probe"
	GOOS=plan9 GOARCH=amd64 "$go_cmd" build -o "$stage/bin/loopback-close-probe" "$repo_root/misc/plan9/arm64/loopback-close-probe.go"
fi

if [ -n "${PLAN9_KERNEL_SRC:-}" ] && [ -f "$PLAN9_KERNEL_SRC/sys/src/9/ip/tcp.c" ]; then
	echo "staging patched 9front tcp.c from $PLAN9_KERNEL_SRC"
	mkdir -p "$stage/misc/plan9/amd64" "$stage/misc/plan9/arm64"
