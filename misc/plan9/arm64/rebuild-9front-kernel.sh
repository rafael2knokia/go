#!/usr/bin/env bash
# Boot a writable 9front/arm64 qcow2 in QEMU, apply the loopback
# tcpsplice fix to /sys/src/9/ip/tcp.c, rebuild the kernel, and
# install the new kernel image into /n/9fat so it boots on next start.
#
...
# misc/plan9/arm64/tcp.c.patched (the patched tcp.c) and
# misc/plan9/arm64/guest-kernel-rebuild.rc (the guest-side build
# script).  After this script completes successfully, the patched
# qcow2 can be passed to run-qemu-tests.sh via PLAN9_IMAGE=...
#
# Reasonably safe: the script operates on a writable qcow2, but it
# does NOT touch the original 9front-11724.arm64.qcow2; the user
# must copy that image to a new file and pass it via -image first.

set -euo pipefail

repo_root=$(git rev-parse --show-toplevel)
image=${PLAN9_IMAGE:-"$repo_root/tmp-9front-test/9front-11724-patched.arm64.qcow2"}
share=${PLAN9_SHARE:-"$repo_root/tmp-9front-goroot"}
share_img=${PLAN9_SHARE_IMG:-"$share.img"}
bios=${PLAN9_UBOOT:-"/usr/lib/u-boot/qemu_arm64/u-boot.bin"}
memory=${PLAN9_MEM:-2048}
smp=${PLAN9_SMP:-4}
cpu=${PLAN9_CPU:-cortex-a72}
guest_script=${PLAN9_TEST_SCRIPT:-"/n/dos/misc/plan9/arm64/guest-kernel-rebuild.rc"}
...
Options:
  ...
  -share PATH    FAT share dir, must contain misc/plan9/arm64/tcp.c.patched
                 (default: $share)
  -bios PATH     U-Boot binary (default: $bios)
...
build_wait=${PLAN9_BUILD_WAIT:-1200}
boot_wait=${PLAN9_BOOT_WAIT:-16}

if [ ! -f "$image" ]; then
	echo "rebuild-9front-kernel.sh: missing writable qcow2 at $image" >&2
	echo "  cp tmp-9front-test/9front-11724.arm64.qcow2 $image first" >&2
	exit 1
fi

if [ ! -d "$share" ] && [ ! -f "$share_img" ]; then
	echo "rebuild-9front-kernel.sh: missing share dir ($share) and image ($share_img)" >&2
	exit 1
fi

if [ ! -f "$share/misc/plan9/arm64/tcp.c.patched" ]; then
	echo "rebuild-9front-kernel.sh: missing $share/misc/plan9/arm64/tcp.c.patched" >&2
	echo "  cp ../9front/sys/src/9/ip/tcp.c $share/misc/plan9/arm64/tcp.c.patched first" >&2
	exit 1
fi
...
(
	...
	-netdev user,id=n0 \
	-device virtio-net-pci-non-transitional,netdev=n0 \
	-nographic \
	-serial mon:stdio
