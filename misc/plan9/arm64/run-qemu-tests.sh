#!/usr/bin/env bash
# Boot 9front/arm64 in QEMU and run the guest compiler test script.

set -euo pipefail

repo_root=$(git rev-parse --show-toplevel)
image=${PLAN9_IMAGE:-"$repo_root/tmp-9front-test/9front-11724.arm64.qcow2"}
...
guest_script=${PLAN9_TEST_SCRIPT:-"/n/dos/misc/plan9/arm64/guest-compiler-tests.rc"}
...
Environment:
  ...
  PLAN9_BOOT_WAIT   seconds before sending login newline (default: 16)
  PLAN9_COPY_WAIT   seconds to wait while copying tools (default: 120)
EOF
}
...
if [ ! -f "$image" ]; then
	...
	echo "run misc/plan9/arm64/fetch-9front-image.sh first" >&2
	exit 1
fi
if [ ! -d "$share" ] && [ ! -f "$share_img" ]; then
	echo "run-qemu-tests.sh: missing share dir ($share) and image ($share_img)" >&2
	echo "run misc/plan9/arm64/make-goroot-share.sh first" >&2
	exit 1
fi
...
if [ ! -f "$bios" ]; then
	echo "run-qemu-tests.sh: missing U-Boot binary: $bios" >&2
	exit 1
fi
...
(
	...
	sleep 2
	printf 'cp /n/dos/pkg/tool/plan9_arm64/* /tmp/gotool\n'
	sleep "$copy_wait"
	...
	sleep 1
	printf 'bind /tmp/gotool /n/dos/pkg/tool/plan9_arm64\n'
	sleep 1
	printf 'cd /n/dos/src\n'
	sleep 1
	printf '%s\n' "$guest_test_cmd"
	...
	-m "$memory" \
	-bios "$bios" \
	-drive if=none,file="$image",format=qcow2,id=hd0,snapshot=on \
	-device qemu-xhci \
